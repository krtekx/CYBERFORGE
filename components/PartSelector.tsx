...
const PartSelector: React.FC<PartSelectorProps> = ({
  ...
  }, [availableParts, activeCategory, categories]);
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 text-[9px] font-black uppercase transition-all border flex items-center gap-2 ${activeCategory === cat ? 'bg-[#ff00ff] text-black border-[#ff00ff]' : 'text-gray-600 border-gray-900 hover:border-[#ff00ff33]'}`}
            >
              <img src={StorageService.getCategoryIcon(cat)} className="w-3 h-3 object-contain" />
              {cat}
            </button>
          ))}
        </div>
